...
extern u8 handMode_flag;
extern u8 warn_temp;
extern u8 BeepIO_State;
extern u8 LED0_IO_State;
extern u8 is_open_beep_form_pj;
extern u8 is_close_beep_form_pj;
extern u8 is_open_led_form_pj;
extern u8 is_close_led_form_pj;
	...
	switch(type)
	{
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				if (!json)UsartPrintf(USART_DEBUG,"Error before: [%s]\r\n",cJSON_GetErrorPtr());	//如果不是json数据
				else
				{
				//处理LED部分:
					json_value = cJSON_GetObjectItem(json , "LED_SW");
					if(strstr(json_value->string,"LED_SW") != NULL)
					{
						UsartPrintf(USART_DEBUG,"json_target: [%s]\r\n",json_value->string);
						UsartPrintf(USART_DEBUG,"json_valueint: [%d]\r\n",json_value->valueint);
						
						
			
						if(json_value->valueint)is_open_led_form_pj = 1;//开灯
						else is_close_led_form_pj = 1;//关灯
					

					}
				//处理蜂鸣器部分:
					json_value = cJSON_GetObjectItem(json , "BEEP_SW");
					if(strstr(json_value->string,"BEEP_SW") != NULL)
					{
						UsartPrintf(USART_DEBUG,"json_target: [%s]\r\n",json_value->string);
						UsartPrintf(USART_DEBUG,"json_valueint: [%d]\r\n",json_value->valueint);
						if(handMode_flag)
						{
							...
							{
								is_open_beep_form_pj = 1;//打开报警器
							}
							else is_close_beep_form_pj = 1;//关闭报警器
						}


					}
				//手动自动模式:
					json_value = cJSON_GetObjectItem(json , "Hand");
					if(strstr(json_value->string,"Hand") != NULL)
					{
						UsartPrintf(USART_DEBUG,"json_target: [%s]\r\n",json_value->string);
						UsartPrintf(USART_DEBUG,"json_valueint: [%d]\r\n",json_value->valueint);
						...
						if(json_value->valueint)
						{
							handMode_flag = 1;//允许手动模式
						}
						else handMode_flag = 0;//进入自动模式
					}
				//处理阈值
					json_value = cJSON_GetObjectItem(json , "Warn");
					if(strstr(json_value->string,"Warn") != NULL)
					{
						UsartPrintf(USART_DEBUG,"json_target: [%s]\r\n",json_value->string);
						UsartPrintf(USART_DEBUG,"json_valueint: [%d]\r\n",json_value->valueint);
						
						if(json_value->valueint)//更新阈值
						{
							warn_temp = json_value->valueint;
						}
					}
